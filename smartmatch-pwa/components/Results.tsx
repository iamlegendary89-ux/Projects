...
import { getSuggestions } from '../services/suggestService';
...
const Results: React.FC<ResultsProps> = ({ initialRecommendations, quizAnswers, allPhones, onRestart, onPhoneSelect }) => {
  ...
  useEffect(() => {
    const refreshRecommendations = async () => {
      ...
      // Try to get personalized suggestions using the new suggest API
      try {
        const personalizedSuggestions = await getSuggestions(
          "best phones based on my quiz preferences",
          location.country,
          userPrefs
        );

        if (personalizedSuggestions.length > 0) {
          setRecommendations(personalizedSuggestions);
          return;
        }
      } catch (error) {
        console.warn('Personalized suggestions failed, falling back to quiz-based:', error);
      }
  ...
  useEffect(() => {
    if (!location?.country || recommendations.length === 0) return;
    ...
    const fetchAllOffers = async () => {
      const offerPromises = recommendations.map(phone =>
        fetch('/api/get-local-offers', {
          ...
          body: JSON.stringify({ phoneName: phone.name, countryCode: location.country }),
