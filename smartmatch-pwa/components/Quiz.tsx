...
const Quiz: React.FC<QuizProps> = ({ onSubmit, onViewAll }) => {
  ...
  const derivePreferencesFromAnswers = (answers: QuizAnswers) => {
    // Derive user preferences from quiz answers for personalization
    const prefs: any = {};

    // OS preference
    if (answers.os !== 'No Preference') {
      prefs.os = answers.os;
    }
  ...
  const handleAnswer = (questionId: keyof QuizAnswers, option: string) => {
    ...
    if (isComplete) {
      // Save preferences derived from quiz answers for personalization
      const preferences = derivePreferencesFromAnswers(newAnswers as QuizAnswers);
