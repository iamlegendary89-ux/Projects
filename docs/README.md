...
│   ├── discovery.ts             # v6 (Production Refactored)
│   ├── enrichment.ts            # v7 (LZFOF Optimized)
│   ├── OSET.ts                  # v6.1 (Hyper-Realistic)
│   ├── sync.ts                  # Writes to BOTH tables
│   ├── truth-loop-analysis.ts   # Feedback -> Weights tuning
│   ├── pipeline.ts              # Orchestrator
│   └── LZFOF/                   # Optimization Benchmarks
│
├── data/
│   ├── phones.json              # Discovery output (basic list)
│   ├── weights.json             # Dynamic attribute weights
│   ├── regret-sentiments.json   # Dynamic regret data
...
## 11. Truth Loop & Feedback System

The **Truth Loop** is an automated system that tunes recommendation weights and regret sentiments based on real user feedback.

### Components

1.  **Feedback Collection**
    *   **Upvotes/Downvotes**: Users vote on phone results.
    *   **Reports**: Users report unexpected regrets (e.g., "Battery worse than expected").
    *   Stored in `feedback` table.

2.  **Analysis Script** (`scripts/truth-loop-analysis.ts`)
    *   Runs nightly.
    *   **Weight Tuning**: Correlates upvotes with phone attributes to adjust `weights.json`.
    *   **Regret Calibration**: Aggregates reports to update `regret-sentiments.json`.
    *   **Self-Healing**: If a phone gets many reports for "Battery", its `battery` score is penalized.
...
3.  **Data Flow**
    ...
    User Feedback (Supabase) 
         │
         ▼
    Truth Loop Analysis ──▶ Updates data/weights.json
         │                   Updates data/regret-sentiments.json
         ▼
    Enrichment/OSET ──────▶ Uses new weights for next batch
    ```
...
| Script | Version | Role | Command |
|--------|---------|------|---------|
| `pipeline.ts` | v1.0 | **Orchestrator** (Sacred Rule #1) | `npm run pipeline` |
| `discovery.ts` | v6 | **Scraper**: Fetches specs & reviews | Part of pipeline |
| `enrichment.ts` | v7 | **AI Engine**: Extracts 7 attributes (uses LZFOF) | Part of pipeline |
| `OSET.ts` | v6.1 | **Normalizer**: Applies time-decay & real-world adjustments | Part of pipeline |
| `sync.ts` | v1.0 | **Syncer**: Updates Dual-State DB | `npm run pipeline:sync` |
| `truth-loop-analysis.ts` | v1.0 | **Feedback Loop**: Auto-tunes weights | Run via cron/scheduler |
